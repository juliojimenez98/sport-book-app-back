...
import { getResourcesByBranch, createResource } from '../controllers/resource.controller';
...
router.put(
  ...
  authorize({ roles: [RoleName.SUPER_ADMIN, RoleName.TENANT_ADMIN, RoleName.BRANCH_ADMIN], branchScope: true }),
  updateBranch
);
...
router.get(
  ...
  authenticate,
  validate(branchIdParamSchema, 'params'),
  authorize({ roles: [RoleName.SUPER_ADMIN, RoleName.TENANT_ADMIN, RoleName.BRANCH_ADMIN, RoleName.STAFF], branchScope: true }),
  getBranchBookings
);
